...
import { getImageTopic } from '../../api/ai_api';
...
export default function Expression() {
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      const response = await getImageTopic();
        ...
        {topics.map(function (element) {
          return (
            <div
              ...
              onClick={() => {
                handleTopicClick(element[0]);
              }}
            >
              <img src={element[1]} className="main-topic-box-img" loading="lazy" alt="주제 사진" />
              <h4>{element[0]}</h4>
            </div>
          );
        })}
